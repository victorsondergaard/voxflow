...
|---|---|---|
| High Accuracy (default) | Best-in-class accuracy, ~100 languages | Apple Silicon: 1–3 s · Intel 2019: 10–25 s |
| Multilingual | Good accuracy, Danish/Spanish/etc. | Apple Silicon: ≤2 s · Intel 2019: 5–15 s |
| English (fast) | Snappy dictation on Intel | Apple Silicon: ≤1 s · Intel 2019: 2–5 s |

Tip for Intel Macs: try High Accuracy first; if the wait annoys you, switch to Multilingual — it still beats native dictation in most real-world use.
...
Everything runs on your Mac: microphone → whisper.cpp (localhost) → optional llama.cpp (localhost) → your text field. The app's only internet use is downloading models from Hugging Face on first run. Nothing is ever uploaded.
